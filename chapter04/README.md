...
- 3 ways of memory management
    ...
    - For Rust, it's managed through a system of ownership with a set of rules that the compiler checks at compile time
...
- The variable is valid from the point at which it’s declared until the end of the current *scope*
  - Example as [Listings 4-1](listings/_01/src/main.rs)

### The String Type
- String literals are convenient, but they are unsuitable for every situation in which we may want to use text, because 
    - They're immutable
...
- Without a GC, it's our responsibility to
...
- In Rust, the memory is automatically returned once the
...
- Example 
    ...
    println!("x = {}, y = {}", x, y);
    ```
    - Types such as integers that have a known size at compile
time are stored entirely on the stack, so copies of the actual values are quick
to make
- `Copy` trait
  - we can place on types like integers that are stored on the stack (more about traits in Chapter 10)
...
- Passing a variable to a function will move or copy, just as assignment does
    - Example as [Listing 4-3](listings/_03/src/main.rs)
### Return Values and Scope
- The ownership of a variable follows the same pattern every time: assigning a value to another variable moves it
- When a variable that includes data on the heap goes out of scope, the value will be cleaned up by `drop` unless the data has been moved to be owned by another variable
    - Example as [Listing 4-4](listings/_04/src/main.rs)
- One workaround would be **moving** the input parameter to one of return values as [Listing 4-4](listings/_05/src/main.rs)
...
- Modifying borrowed immutable values wouldn't work
  - Example as [Listing 4-6](listings/_06/src/main.rs)
- References are immutable by default

### Mutable References
- One big retriction: you can have only one mutable reference to a particular piece of data in a particular scope
...
### Dangling References
- *Dangling pointer* references a location in memory that may have been given to someone else, by freeing some memory while preserving a pointer to that memory
...
- Requirement: write a function that takes a string and
returns the first word it finds in that string. If the function doesn't find a
space in the string, the whole string must be one word, so the entire string
should be returned.
- Level-1 solution as [Listing 4-8](listings/_08/src/main.rs)
...
`[starting_index..ending_index]`, where `starting_index` (if omitted, default as 0) is the first position
in the slice and `ending_index` (if omitted, default as the length of being slicing) is one more than the last position in the slice
...
- Level-3 solution for the *small programming problem* as [Listing 4-9](listings/_09/src/main.rs)
